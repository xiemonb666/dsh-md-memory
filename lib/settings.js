...
export function budgetForRoute(provider, model, caps = {}, base = {}) {
	...
	// A backend capability probe may know the context window but not expose an
	// output limit.  Treat maxTokens=0 as "unknown" rather than as a real
	// zero-token limit; otherwise the request guard clamps every request to
	// zero and rejects even a tiny prompt.
	const defaultOutput = route.maxOutputTokens ?? (adapterMax || base.maxTokens || 1);
	return {
		contextWindow,
		targetContextTokens: Math.min(route.targetContextTokens ?? contextWindow, contextWindow || Number.MAX_SAFE_INTEGER),
		maxOutputTokens: Math.min(defaultOutput, adapterMax || Number.MAX_SAFE_INTEGER),
